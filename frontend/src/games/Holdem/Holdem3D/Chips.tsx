...
const Chips: React.FC<ChipsProps> = ({ amount, position, fromPosition, delay = 0 }) => {
  ...
  const { springPos } = useSpring({
    from: { springPos: fromPosition || [position[0], position[1] + 5, position[2]] }, // Drop from sky if no fromPosition
    to: { springPos: position },
    delay,
    config: { mass: 1, tension: 120, friction: 20 }, // Bouncy drop
  });

  return (
    <animated.group position={springPos as any}>
