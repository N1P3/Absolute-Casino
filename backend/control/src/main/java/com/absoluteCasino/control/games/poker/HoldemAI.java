...
public class HoldemAI {
    ...
    public AIAction predictMove(HoldemTable table, Seat aiSeat) {
        ...
        try {
            ...
            // 5. Decode Action
            AIAction action = decodeAction(actionProbs, predictedValue, hand, table);
    ...
    private AIAction playBasicStrategy(HoldemTable table, HoldemHand hand, Seat aiSeat) {
        ...
        long toCall = hand.getCurrentBet() - hand.getBetThisStreetForSeat(aiSeat.getPosition());
    ...
    private AIAction decodeAction(float[] probs, float valPred, HoldemHand hand, HoldemTable table) {
        ...
        switch (bestIdx) {
            ...
            case 1:
                action.type = PlayerActionType.CALL;
                // Check if Check is valid instead of Call
                if (hand.getCurrentBet() == 0
                        || hand.getBetThisStreetForSeat(table.getCurrentPlayerSeat()) == hand.getCurrentBet()) {
                    action.type = PlayerActionType.CHECK;
                }
                break;
            case 2:
