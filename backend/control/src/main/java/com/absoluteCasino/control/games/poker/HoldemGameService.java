...
public class HoldemGameService {
	...
	public HoldemTable createTableIfAbsent(int tableId) {
		return tables.computeIfAbsent(tableId,
				id -> new HoldemTable(id, maxSeatsPerTable, new Blinds(10, 20), 1000L, 4000L));
	}
	...
	public synchronized void joinTable(int tableId, Long userId, long buyIn) {
		...
		Seat existingSeat = table.getSeats().stream()
				.filter(seat -> seat.isOccupied() && userId.equals(seat.getUserId())).findFirst().orElse(null);
		...
		Seat freeSeat = table.getSeats().stream().filter(seat -> !seat.isOccupied()).findFirst().orElse(null);
		...
		if (freeSeat != null && !handInProgress) {
			...
			waitingPlayers.computeIfAbsent(tableId, id -> new ArrayList<>()).add(new WaitingPlayer(userId, buyIn));
		}
	}
	...
	public synchronized void startHandIfPossible(int tableId) {
		...
		List<Seat> activeSeats = table.getSeats().stream().filter(Seat::isOccupied).filter(seat -> !seat.isSittingOut())
				.toList();
	...
	public synchronized String handlePlayerAction(int tableId, Long userId, PlayerActionType action, long amount) {
		...
		case CALL -> {
			...
			hand.getActionHistory().add(new HoldemHand.ActionRecord(seat.getPosition(), PlayerActionType.CALL, toCall));
		}
		case BET -> {
	...
	private Seat findSeatByUserId(HoldemTable table, Long userId) {
		return table.getSeats().stream().filter(Seat::isOccupied).filter(seat -> userId.equals(seat.getUserId()))
	...
	private Seat getNextOccupiedSeat(HoldemTable table, int startPosition, List<Seat> activeSeats) {
		int maxSeats = table.getMaxSeats();
		int pos = startPosition;
		while (true) {
			pos = (pos + 1) % maxSeats;
			int finalPos = pos;
			Seat seat = activeSeats.stream().filter(s -> s.getPosition() == finalPos).findFirst().orElse(null);
			if (seat != null) {
				return seat;
			}
		}
	...
	private void postBlindsAndDeal(HoldemTable table, HoldemHand hand, List<Seat> activeSeats) {
	...
	private void takeChips(HoldemHand hand, Seat seat, long amount) {
	...
	private boolean isHandEndedByFolds(HoldemHand hand) {
		long notFolded = hand.getPlayers().values().stream().filter(ps -> !ps.isFolded()).count();
		return notFolded <= 1;
	}
	...
	private boolean isBettingRoundEnded(HoldemTable table, HoldemHand hand) {
		...
		if (hand.getCurrentBet() == 0) {
			...
			long toActCount = active.stream().filter(ps -> ps.getChipsInPotThisStreet() != hand.getCurrentBet())
					.count();
			return toActCount == 0;
		}
	}
	...
	private void moveToNextPlayer(HoldemTable table, HoldemHand hand) {
		...
		if (activeSeats.isEmpty()) {
			...
			long notFoldedCount = hand.getPlayers().values().stream().filter(ps -> !ps.isFolded()).count();
		...
		while (true) {
			pos = (pos + 1) % maxSeats;
			int finalPos = pos;
			boolean found = activeSeats.stream().anyMatch(s -> s.getPosition() == finalPos);
			if (found) {
	...
	private void finishHandByFolds(HoldemTable table, HoldemHand hand) {
		PlayerHandState winner = hand.getPlayers().values().stream().filter(ps -> !ps.isFolded()).findFirst()
				.orElse(null);
		...
		if (winner != null) {
			Seat winnerSeat = table.getSeats().stream().filter(seat -> seat.getPosition() == winner.getSeatPosition())
	...
	private void finishHandByShowdown(HoldemTable table, HoldemHand hand) {
		List<PlayerHandState> contenders = hand.getPlayers().values().stream().filter(ps -> !ps.isFolded()).toList();
		...
		for (PlayerHandState ps : contenders) {
			Seat seat = table.getSeats().stream().filter(s -> s.getPosition() == ps.getSeatPosition()).findFirst()
					.orElse(null);
		...
		for (SidePot pot : pots) {
			List<PlayerHandState> eligible = contenders.stream().filter(ps -> ps.getTotalChipsInPot() > 0)
					.filter(ps -> pot.eligibleSeats.contains(ps.getSeatPosition())).toList();
			...
			for (int i = 0; i < winners.size(); i++) {
				...
				Seat winnerSeat = table.getSeats().stream().filter(seat -> seat.getPosition() == ps.getSeatPosition())
						.findFirst().orElse(null);
				if (winnerSeat != null) {
	...
	private List<SidePot> buildSidePots(Map<Integer, Long> committed) {
		...
		while (true) {
			long min = remaining.values().stream().filter(v -> v > 0).min(Long::compareTo).orElse(0L);
	...
	private void endHandAndMoveWaitingPlayers(HoldemTable table) {
		...
		removeBustedAiBots(table);
		moveWaitingPlayersToSeats(table);
	}

	private void removeBustedAiBots(HoldemTable table) {
		// Remove AI bots that have run out of money
		for (Seat seat : table.getSeats()) {
			if (seat.isOccupied() && seat.isAi() && seat.getStack() <= 0) {
				System.out.println("Removing busted AI bot from seat " + seat.getPosition());
				seat.setUserId(null);
				seat.setStack(0);
				seat.setSittingOut(false);
			}
